...
#
# Author: Javilonas (Javier Sayago)
# Support: www.lonasdigital.com
# based on original source the plugin StreamTV
# Source code: http://code.vuplus.com/gitweb/?p=vuplus_dvbapp;a=tree;f=lib/python/Plugins/Extensions/StreamTV
#
#endif
...
PLUGIN_PATH = resolveFilename(SCOPE_PLUGINS, "Extensions/OpenIPTV")

class OpenIPTVPlayer(Screen, InfoBarNotifications):
	skin = 	"""
		<screen name="OpenIPTV Player" flags="wfNoBorder" position="0,570" size="1280,190" title="OpenIPTV Player" backgroundColor="#41000000" >
			<ePixmap position="80,25" size="117,72" pixmap="%s/picon.png" zPosition="-1" transparent="1" alphatest="blend" />
...
def main(session, **kwargs):
	session.open(OpenIPTVList)

def Plugins(**kwargs):
	return PluginDescriptor(name=_("OpenIPTV Player"), description="IPTV free channels.", where = PluginDescriptor.WHERE_PLUGINMENU, icon = "plugin.png", fnc=main)
