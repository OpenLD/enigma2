...
class SetupCronConf(Screen, ConfigListScreen):
	...
	def createSetup(self):
		...
		if config.ldteam.cronmanager_commandtype.value == 'custom':
	...
	def checkentry(self):
		msg = ''
		if (config.ldteam.cronmanager_commandtype.value == 'predefined' and config.ldteam.cronmanager_predefined_command.value == '') or config.ldteam.cronmanager_commandtype.value == 'custom' and config.ldteam.cronmanager_user_command.value == '':
	...
	def saveMycron(self):
		hour = '%02d' % config.ldteam.cronmanager_cmdtime.value[0]
		minutes = '%02d' % config.ldteam.cronmanager_cmdtime.value[1]
		if config.ldteam.cronmanager_commandtype.value == 'predefined' and config.ldteam.cronmanager_predefined_command.value != '':
			command = config.ldteam.cronmanager_predefined_command.value
		else:
			command = config.ldteam.cronmanager_user_command.value

		if config.ldteam.cronmanager_runwhen.value == 'Hourly':
			newcron = minutes + ' ' + ' * * * * ' + command.strip() + '\n'
		elif config.ldteam.cronmanager_runwhen.value == 'Daily':
			newcron = minutes + ' ' + hour + ' * * * ' + command.strip() + '\n'
		elif config.ldteam.cronmanager_runwhen.value == 'Weekly':
			if config.ldteam.cronmanager_dayofweek.value == 'Sunday':
				newcron = minutes + ' ' + hour + ' * * 0 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Monday':
				newcron = minutes + ' ' + hour + ' * * 1 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Tuesday':
				newcron = minutes + ' ' + hour + ' * * 2 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Wednesday':
				newcron = minutes + ' ' + hour + ' * * 3 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Thursday':
				newcron = minutes + ' ' + hour + ' * * 4 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Friday':
				newcron = minutes + ' ' + hour + ' * * 5 ' + command.strip() + '\n'
			elif config.ldteam.cronmanager_dayofweek.value == 'Saturday':
				newcron = minutes + ' ' + hour + ' * * 6 ' + command.strip() + '\n'
		elif config.ldteam.cronmanager_runwhen.value == 'Monthly':
			newcron = minutes + ' ' + hour + ' ' + str(config.ldteam.cronmanager_dayofmonth.value) + ' * * ' + command.strip() + '\n'
		else:
			command = config.ldteam.cronmanager_user_command.value
		...
		out.close()
		rc = system('crontab /etc/cron/crontabs/root -c /etc/cron/crontabs')
		config.ldteam.cronmanager_predefined_command.value = 'None'
		config.ldteam.cronmanager_user_command.value = 'None'
		config.ldteam.cronmanager_runwhen.value = 'Daily'
		config.ldteam.cronmanager_dayofweek.value = 'Monday'
		config.ldteam.cronmanager_dayofmonth.value = "1"
		config.ldteam.cronmanager_cmdtime.value, mytmpt = ([0, 0], [0, 0])
		self.close()
