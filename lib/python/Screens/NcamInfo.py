...
class NcamInfo:
	...
	def confPath(self):
		...
		#cmd = 'find %s -name "ncam.conf"' % sdirs
		#res = os.popen(cmd).read()
		#if res == "":
		#	return None
		#else:
		#	return res.replace("\n", "")
		cmd = 'ps -eo command | sort -u | grep -v "grep" | grep -c "ncam"'
		res = os.popen(cmd).read()
		if res:
			data = res.replace("\n", "")
			if int(data) == 1:
				cmd = 'ps -eo command | sort -u | grep -v "grep" | grep "ncam"'
				res = os.popen(cmd).read()
				if res:
					data = res.replace("\n", "")
					...
					try:
						...
						try:
							print 'NCaminfo - ncam start-command is not as "/ncam-binary -parameter /config-folder" executed, using hard-coded config dir'
							cmd = binary + ' -V | grep ConfigDir'
							res = os.popen(cmd).read()
							data = res.split(":")[1]
						except:
							print 'NCaminfo - ncam binary appears to be broken'
							return None
					data = data.strip() + '/ncam.conf'
					if os.path.exists(data):
						print 'NCaminfo - config file "%s" ' % data
						return data
					print 'NCaminfo - config file "%s" not found' % data
					return None
			elif int(data) > 1:
				print 'NCaminfo - more than one(%s) ncam binarys is active'  % data
				return None
		print 'NCaminfo - no active ncam binarys found'
		return None

	def getUserData(self):
		err = ""
		self.ncamconf = self.confPath()
		self.username = ""
		self.password = ""
		if self.ncamconf is not None:
			...
			if not webif:
				err = _("There is no [webif] section in ncam.conf")
			elif not httpuser:
				err = _("No httpuser defined in ncam.conf")
			elif not httppwd:
				err = _("No httppwd defined in ncam.conf")
			elif not httpport:
				err = _("No httpport defined in ncam.conf. This value is required!")
			...
			return _("file ncam.conf could not be found")
