...
class SpaceInfo(Poll, Converter):
	HDDTEMP = 0
	LOADAVG = 1
	MEMTOTAL = 2
	MEMFREE = 3
	...
	def __init__(self, type):
		...
		if self.type in (self.FLASHINFO,self.HDDINFO,self.USBINFO,self.MMCINFO):
	...
	def getText(self):
		...
		if self.type == self.HDDTEMP:
			...
			if self.type in (self.USBINFO,self.HDDINFO,self.MMCINFO,self.FLASHINFO):
	...
	def getValue(self):
		...
		elif self.type in (self.USBINFO,self.HDDINFO,self.MMCINFO,self.FLASHINFO):
			path = {self.USBINFO: "/media/usb", self.HDDINFO: "/media/hdd", self.MMCINFO: "/media/mmc", self.FLASHINFO: "/"}[self.type]
